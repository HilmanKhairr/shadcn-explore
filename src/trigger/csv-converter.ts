...
export const processCsvPipelineTask = task({
  ...
  run: async (payload: CsvPipelinePayload): Promise<CsvPipelineResult> => {
    ...
    logAndSave(`Config: ${JSON.stringify(payload.filterConfig)}`);
    ...
    logAndSave(
      `Step 2/3: Applying filter (${payload.filterConfig.filterColumn} = "${payload.filterConfig.filterValue}") & text mutation`
    );
    await wait.for({ seconds: 1 });
      ...
      content = JSON.stringify(
        {
          ...
          configApplied: payload.filterConfig,
          records: [
            {
              id: 101,
              name: "PRODUCT ALPHA",
...
0 -30 Td
/F1 12 Tf
(Orientation: ${payload.filterConfig.pdfOrientation || "portrait"}) Tj
0 -20 Td
(Filter: ${payload.filterConfig.filterColumn} = ${payload.filterConfig.filterValue}) Tj
ET
endstream endobj
xref
0 6
