...
import type { FilterFileNodeData } from "@/pages/reactflow-showcase/types/pipeline";
import { Position, type Node, type NodeProps } from "@xyflow/react";
import { AlertCircle, FileSpreadsheet, SlidersHorizontal } from "lucide-react";
import { memo } from "react";
import { useShallow } from "zustand/react/shallow";
import { EDGE_CLASSIFICATION } from "../../constants/pipeline";
import DashedCircleHandle from "../handle/DashedCircleHadle";

export type FilterFileNodeProps = Node<FilterFileNodeData, "filterNode">;
...
const FilterFileNode = memo(
  ({ id, data, selected }: NodeProps<FilterFileNodeProps>) => {
    ...
    const upstreamFileInfo = usePipelineStore(
      useShallow((state) => {
        ...
        const firstItemWithFile = dataPayload.find(
          (item) => item.file && item.file.status !== "error"
        );
        return firstItemWithFile?.file || null;
      })
    );

    const hasUpstreamData = !!upstreamFileInfo;
    const filterColumn = data.filterColumn || "";
    const filterOperation = data.filterOperation || "equals";
    const filterValue = data.filterValue || "";
    const limitRows =
      data.limitRows !== undefined ? String(data.limitRows) : "100";
    const removeEmptyRows = data.removeEmptyRows ?? true;
    const trimWhitespace = data.trimWhitespace ?? true;
        ...
        {!hasUpstreamData ? (
          <div className="my-2 flex flex-col items-center rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-center">
            ...
            <p className="text-muted-foreground mt-0.5 text-[10px] leading-relaxed">
              Hubungkan garis & upload file CSV pada Input Node terlebih dahulu
              untuk mengaktifkan opsi konfigurasi.
            </p>
          </div>
        ) : (
          <div className="nodrag space-y-2.5 text-left">
            <div className="flex items-center justify-between rounded-lg border border-purple-500/20 bg-purple-500/10 px-2.5 py-1.5 text-[10px]">
              <div className="flex items-center gap-1.5 truncate">
                <FileSpreadsheet className="size-3.5 shrink-0 text-purple-400" />
                <span className="text-foreground truncate font-semibold">
                  {upstreamFileInfo.fileName}
                </span>
              </div>
              <Badge
                variant="secondary"
                className="shrink-0 font-mono text-[9px] text-purple-400"
              >
                {upstreamFileInfo.fileSize}
              </Badge>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={filterColumn}
                onChange={(e) => handleUpdate("filterColumn", e.target.value)}
                placeholder="e.g. status, user_id, price..."
                className="bg-background border-border text-foreground placeholder:text-muted-foreground/60 h-7 w-full rounded-md border px-2 font-mono text-xs transition-colors focus:border-purple-500 focus:outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <CompactSelect
                  ...
                  onValueChange={(val) => handleUpdate("filterOperation", val)}
                />
              </div>
              <div>
                <label className="text-muted-foreground mb-1 block text-[10px] font-semibold">
                ...
                <input
                  type="text"
                  value={filterValue}
                  onChange={(e) => handleUpdate("filterValue", e.target.value)}
                  placeholder="Value..."
                  className="bg-background border-border text-foreground h-7 w-full rounded-md border px-2 text-xs transition-colors focus:border-purple-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <CompactSelect
                ...
                value={limitRows}
                items={limitOptions}
                onValueChange={(val) => handleUpdate("limitRows", Number(val))}
              />
            </div>

            <div className="border-border/50 space-y-1.5 border-t pt-2 text-[10px]">
              <label className="flex cursor-pointer items-center justify-between">
                <span className="text-muted-foreground">Remove Empty Rows</span>
                <input
                  type="checkbox"
